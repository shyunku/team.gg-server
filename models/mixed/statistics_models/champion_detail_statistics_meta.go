...
func GetChampionDetailStatisticsMetaMXDAOs_optimized(db db.Context, matchGameVersions []string) ([]ChampionDetailStatisticsMetaMXDAO, error) {
	if len(matchGameVersions) == 0 {
		return nil, errors.New("match game versions are required")
	}
	...
	totalSqls := []string{
		...
		`CREATE TEMPORARY TABLE IF NOT EXISTS MainGroup AS 
			SELECT match_participant_id, champion_id, champion_name, team_position, 
		...
		`CREATE TEMPORARY TABLE IF NOT EXISTS RefinedMetaGroups AS 
			SELECT champion_id, champion_name, team_position, primary_style, sub_style, item0_id, item1_id, item2_id, SUM(wins) AS wins, SUM(count) AS total, SUM(wins) / SUM(count) AS win_rate
			FROM MainGroup
			WHERE item0_id IS NOT NULL AND item1_id IS NOT NULL AND item2_id IS NOT NULL
	...
	dropSqls := []string{
		`DROP TEMPORARY TABLE IF EXISTS SortedPerks;`,
		`DROP TEMPORARY TABLE IF EXISTS PerkGroups;`,
		`DROP TEMPORARY TABLE IF EXISTS PerkStyleGroups;`,
		`DROP TEMPORARY TABLE IF EXISTS ItemDetails;`,
		`DROP TEMPORARY TABLE IF EXISTS MainGroup;`,
		`DROP TEMPORARY TABLE IF EXISTS SummonerSpellCounts;`,
		`DROP TEMPORARY TABLE IF EXISTS SummonerSpellRanks;`,
		`DROP TEMPORARY TABLE IF EXISTS PerkCounts;`,
		`DROP TEMPORARY TABLE IF EXISTS PerkRanks;`,
		`DROP TEMPORARY TABLE IF EXISTS FullItemTreeGroups;`,
		`DROP TEMPORARY TABLE IF EXISTS FullItemTreeRanks;`,
		`DROP TEMPORARY TABLE IF EXISTS RefinedMetaGroups;`,
		`DROP TEMPORARY TABLE IF EXISTS RankedMetas;`,
		`DROP TEMPORARY TABLE IF EXISTS FinalRankedMetas;`,
	}
